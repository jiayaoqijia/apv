...
pdf_load_version(pdf_xref *xref)
{
	...
	//read one line end by '\n\r'
	fz_read_line(xref->file, buf, sizeof buf);

	//decryption
	//__android_log_write(ANDROID_LOG_ERROR, "pdf_load_version, buf", buf);//Or ANDROID_LOG_INFO, ...
	/*count = sizeof buf;
	while(count--)
	{
		//tmp[count] = buf[count] ^ KEY;
		buf[count] = buf[count] ^ KEY;
	}
	__android_log_write(ANDROID_LOG_ERROR, "pdf_load_version, Decrypt buf", buf);//Or ANDROID_LOG_INFO, ...
	*/

...
fz_read(fz_stream *stm, unsigned char *buf, int len)
{
	...
	if (count)
	{
		...
		stm->rp += count;
		//return fz_rethrow(2, "fz_read buf error");
		//printf("fz_read buf is %s", buf);
		//tmp = (char*) malloc (count+1);
		//memcpy(tmp, buf, count);
		/*count1 = count;
		while(count1--)
		{
			buf[count1 + 1] = buf[count1 + 1] ^ mask;
		}*/
		/*count1 = count;
		while(count1--)
		{
		...
		//sprintf(strlen, "%d", count);
		//__android_log_write(ANDROID_LOG_ERROR, "stm_read", buf);//Or ANDROID_LOG_INFO, ...
		//free(tmp);
		//__android_log_write(ANDROID_LOG_ERROR, "stm_read", strlen);//Or ANDROID_LOG_INFO, ...
	}
